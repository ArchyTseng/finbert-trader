...
class DataResource:
    ...
    def clean_yf_ohlcv(self, df, symbol):
        ...
        try:
            ...
            if len(df) < len(df_original):
                logging.info(f"DR Module - Filtered out {len(df_original) - len(df)} rows with Adj_Close <= 0")  # Log filtered rows for data integrity tracking
            return df  # Return the cleaned and filtered DF
        except Exception as e:
            logging.warning(f"DR Module - Error cleaning OHLCV data for {symbol}: {e}")  # Log any errors during cleaning)
    ...
    def fetch_stock_data(self):
        ...
        for symbol in self.config.symbols:
            ...
            try:
                ...
                if os.path.exists(cache_path):
                    ...
                    for attempt in range(3):  # Retry loop for network issues to enhance reliability
                        try:
                            ...
                            logging.warning(f"DR Module - Download attempt {attempt+1} failed for {symbol}: {e}; retrying in 5s")  # Log retry warning
                    ...
                    if 'Symbol' in df.columns:
                        ...
                        logging.info(f"DR Module - Dropped 'Symbol' column for {symbol}")  # Log drop for auditing
                    ...
                    logging.info(f"DR Module - Stock {symbol} Date dtype: {df['Date'].dtype}")  # Log final dtype for verification
                ...
                logging.info(f"DR Module - Successfully download stock data for {symbol}")  # Log success per symbol
            except Exception as e:
                logging.error(f"DR Module - Error downloading {symbol}: {e}")  # Log any unhandled errors
        logging.info(f"DR Module - Return stock data for {self.config.symbols} as {type(stock_data_dict)}")  # Log final return type and symbols
    ...
    def cache_path_config(self, base_path='nasdaq_exteral_data.csv'):
        ...
        logging.info(f"DR Module - Cache path: {cache_path}, Filtered cache path: {filtered_cache_path}")  # Log paths for monitoring and debugging
    ...
    def load_news_data(self, cache_path, filtered_cache_path, chunksize=None):
        ...
        if cache_path is None or filtered_cache_path is None:
            ...
            logging.error("DR Module - Cache path is needed. Filtered cache path is needed.")
            return
        def clean_chunk_tz(chunk):
            ...
            if 'Date' in chunk.columns:
                ...
                logging.info("DR Module - Removed timezone from Date in chunk")  # Log timezone cleanup
            else:
                logging.info("DR Module - 'Date' column not found in chunk")  # Log if Date missing
        ...
        if os.path.exists(filtered_cache_path):
            ...
            logging.info(f"DR Module - Loading from filtered cache: {filtered_cache_path}")
            try:
                ...
                if filtered_df['Date'].dtype != 'datetime64[ns]':   
                    ...
                    logging.info("DR Module - Removed timezone from Date in filtered cache")  # Log adjustment
                ...
                logging.info(f"DR Module - Loaded {len(filtered_df)} rows from filtered cache")  # Log loaded rows
                return  # Skip further processing after yielding
            except Exception as e:
                logging.error(f"DR Module - Error loading filtered cache: {e}. Falling back to original processing.")  # Log error and fallback
        ...
        if not os.path.exists(cache_path):
            ...
            if downloaded_path is None:
                logging.error("DR Module - Download failed. Aborting load_news_data.")  # Log download failure
                return  # Exit if download fails

        # Initial Chunksize and Chunk Reader
        chunksize = chunksize or self.config.chunksize  # Use provided or config chunksize
        logging.info(f"DR Module - Chunksize: {chunksize}")  # Log chunksize for monitoring
        try:
            chunks = pd.read_csv(
                cache_path,
                chunksize=chunksize,
            ...
            logging.info(f"DR Module - Loaded chunks from {cache_path}")  # Log reader init
        except Exception as e:
            logging.error(f"DR Module - Error initializing CSV reader: {e}")  # Log reader error
        ...
        logging.info(f"DR Module - Start to filter news data")  # Log start of filtering
        try:
            for chunk in tqdm(chunks, desc="Loading News Chunks"):
                ...
                logging.info(f"DR Module - Chunk filtered to global date range: {self.train_start_date} to {self.test_end_date}, {len(chunk)} rows")  # Log post-date filter size
            ...
            logging.error(f"DR Module - Error during chunk processing: {e}")  # Log processing error
        ...
        if filtered_chunks:
            ...
            logging.info(f"DR Module - Filtered news data saved to {filtered_cache_path}")  # Log save success